...
const Billing = () => {
  ...
  const handleProductSelect = (itemIndex, stockId) => {
    ...
    if (selectedCustomer) {
      customerData = customers.filter(
        (item, index) => item.customer_id === selectedCustomer.customer_id
      );
    } else {
    ...
    const filterMasterItem =
      customerData[0].MasterJewelTypeCustomerValue.filter(
        (item, index) => item.masterJewel_id === selectedProduct.master_jewel_id
      );
    if (filterMasterItem.length === 0) {
      ...
      billObj.productPure =
        ((billObj.productTouch + filterMasterItem[0].value) *
  ...
  const handleBalanceInputChange = (index, field, value) => {
    ...
    if (field === "touch" || field === "givenGold") {
      updatedRows[index]["pure"] =
        (updatedRows[index]["givenGold"] * updatedRows[index]["touch"]) / 100;
    }
  ...
  const handleChangePercentage = (itemIndex, value) => {
    ...
    filteredbill[0].productPure =
      ((filteredbill[0].productTouch + parseInt(value)) *
        filteredbill[0].productWeight) /
      100;
          ...
          <Table>
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell>
                  {(balanceRow.length === 0 ? totalPrice : closing).toFixed(2)}
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
            ...
            {productWeight.length > 0 ? (
              productWeight.map((product, index) => (
                <TableRow
                  key={index}
                  onClick={() => {
                    handleProductSelect(index, product.stock_id);
                  }}
