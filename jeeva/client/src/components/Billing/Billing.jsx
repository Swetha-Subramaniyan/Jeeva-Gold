...
//                   <strong>Total</strong>
//                 </td>
//                 <td className="td"><strong>{totalNo}
//                   </strong></td>
//                 <td className="td"></td>
//                 <td className="td">
...
const Billing = () => {
  ...
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    if (["no", "percentage", "touch"].includes(name)) {
  ...
  useEffect(() => {
    if (newItem.name && newItem.no && newItem.percentage) {
      ...
      const stockItem = stocks.find(
        (stock) => stock.coinType.toString() === newItem.name.toString()
      );
      if (stockItem) {
        const availableQuantity = stockItem.quantity;
        if (no > availableQuantity) {
          alert(
            `Insufficient stock available for Coin value ${newItem.name}. Available: ${availableQuantity}`
          );
          setNewItem((prev) => ({
            ...prev,
            no: availableQuantity.toString(),
            weight: (coin * availableQuantity).toString(),
            pure:
              pure % 1 === 0
                ? (coin * availableQuantity * (percentage / 100)).toString()
                : (coin * availableQuantity * (percentage / 100)).toFixed(3),
          }));
          return;
        }
      }
  ...
  }, [newItem.name, newItem.no, newItem.percentage, stocks]);

  const handleSaveItem = () => {
    if (!newItem.name || !newItem.no || !newItem.percentage) {
      alert("Please fill in Coin Value, No, and Percentage fields.");
      return;
    }
    ...
    if (stockItem) {
      const requestedQuantity = parseFloat(newItem.no);
      if (requestedQuantity > stockItem.quantity) {
        alert(
          `Insufficient stock available for Coin value ${newItem.name}. Available: ${stockItem.quantity}`
        );
        return;
      }
    } else {
      alert(`Stock for Coin value ${newItem.name} not found.`);
      return;
    }
    ...
    const updatedStocks = stocks.map((stock) => {
      if (stock.coinType.toString() === newItem.name.toString()) {
        return {
          ...stock,
          quantity: stock.quantity - parseFloat(newItem.no),
        };
      }
      return stock;
    });
  ...
  const calculateTotals = () => {
    ...
    billItems.forEach((item) => {
      ...
      totalPurity += parseFloat(item.pure) || 0;
      if (goldRate) {
        totalAmount += parseFloat(item.pure) * parseFloat(goldRate);
      }
      totalNo += parseFloat(item.no) || 0;
    });
  ...
  return (
    <>
      ...
      <Box className="add-customer-container">
        <AddCustomer onAddCustomer={(newCustomer) => {
              ...
              {billItems.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="td">
                    {goldRate
                      ? (
                        parseFloat(item.pure) * parseFloat(goldRate)
                      ).toFixed(2)
                      : "-"}
                  </td>
                </tr>
      ...
      >
        <Box className="modal-container">
          ...
          <Box component="form" className="modal-form" noValidate autoComplete="off">
            <TextField
              fullWidth
              label="Coin Value"
              name="name"
